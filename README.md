...
* 为了方便注入 bug,节约大家的时间，请修改相关 .gitignore 文件，将 navy-apps/apps/pal/repo 也上传到自己的在线合库(含 repo/data)，考核通过后可将这部分从在线仓库中删除。注意在修改 .gitignore 文件的时候仍需要将每级的 build 文件夹排除在外（即不上传）。
